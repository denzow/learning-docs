...
ポートレートのライティングには、鼻の影の形で名前の付いた型がある。
...
広く見える側にキーライトが当たるのが**ブロードライティング**、狭く見える側に当たるのが**ショートライティング**である。
...
カメラは第 3 章の基準どおり、M モード、ISO 100、1/200 秒、f/8、ホワイトバランス 5500K、RAW、メカシャッター、露出 Simulation は「しない」に設定する。
...
背景を明るくしたければ、被写体と背景を近づける（背景まで 1.5m なら段差はほぼ 0 になるが、被写体の影が背景に落ちる）か、第 7 章の背景ライトを足す。
